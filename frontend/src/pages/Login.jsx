...
const InputField = ({ label, icon: Icon, ...props }) => (
  ...
  >
    ...
    <div className="relative flex items-center bg-white/5 border border-white/20 rounded-xl hover:border-purple-400 transition-all">
      <Icon className="w-5 h-5 text-purple-300 ml-3" />
      <input
        {...props}
...
const Login = () => {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-[#1a1a2e] via-[#16213e] to-[#0f3460] px-4 relative overflow-hidden">
      ...
      >
        <motion.div
          className="absolute w-72 h-72 bg-purple-500/20 rounded-full blur-3xl top-10 left-14"
          animate={{ y: [0, 20, 0] }}
          transition={{ duration: 6, repeat: Infinity }}
        />
        <motion.div
          className="absolute w-96 h-96 bg-blue-500/10 rounded-full blur-3xl bottom-14 right-10"
          animate={{ y: [0, -20, 0] }}
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <motion.button
            ...
            className={`w-full py-3 rounded-xl font-semibold flex items-center justify-center gap-2 transition-all shadow-lg 
              ${submitting ? "bg-gray-600 cursor-not-allowed" : "bg-gradient-to-r from-purple-500 to-blue-500 hover:from-blue-500 hover:to-purple-500"}
            `}
          >
            {submitting ? (
              <>
